...
const AddMember: React.FC<AddMemberProps> = ({ projectId, onMemberAdded }) => {
  ...
  return (
    <div style={{ maxWidth: 320 }}>
      <label htmlFor="usernameInput">Kullanıcı Adı Giriniz:</label>
      <input
        type="text"
        id="usernameInput"
        value={username}
        ...
        style={{ width: '100%', padding: 8, margin: '8px 0',textAlign:'start', fontSize: '16px' }}
      />
      <button
        onClick={handleAddMember}
        disabled={loading}
        style={{ width: '100%', padding: 10, fontSize:'15px', fontWeight:'600',backgroundColor: '#6d84f6ff' , cursor: loading ? 'not-allowed' : 'pointer' }}
      >
