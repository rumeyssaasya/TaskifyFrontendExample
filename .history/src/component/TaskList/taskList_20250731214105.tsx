...
const statusLabels = {
  ...
  DONE: "Tamamlandı",
  COMPLETED: "Tamamlandı" // Added for consistency
};
...
const getStatusClass = (status: string): string => {
  const statusClasses = {
    ...
    COMPLETED: "done",
    DONE: "done" // Added to handle both DONE and COMPLETED
  };
            ...
            <div key={task.id} className={`task-card ${getStatusClass(task.status)}`}>
              <div className="task-content">
                ...
                <span className={`task-status ${getStatusClass(task.status)}`}>
                  {statusLabels[task.status as keyof typeof statusLabels] || "Bilinmiyor"}
                </span>
              </div>
