...
function ProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Backend'den gelen profileImageUrl artık path olarak (örn: /images/2.png)
      const imagePath = data.profileImageUrl?.startsWith('/images/')
          ? data.profileImageUrl
          : data.profileImageUrl?.split('/').slice(-2).join('/');

      if (imagePath && profileImageOptions.includes(`/${imagePath}`)) {
          setTempProfileImage(`${backendBaseUrl}/${imagePath}`);
      } else {
          setTempProfileImage(`${backendBaseUrl}/images/defaultIcon.png`);
      }
      } catch {
  ...
  const handleEditToggle = () => {
    if (editMode && profile) {
      ...
      if (profile.profileImageUrl && profileImageOptions.includes(profile.profileImageUrl)) {
  ...
  const handleSave = async () => {
    ...
    try {
      // Seçilen resmi backend’e göndermek için sadece path kısmını ayır
      ...
      <div className="profile-image-wrapper">
        <div className="profile-image-box">
          <img
            ...
            onError={e => e.currentTarget.src = `${backendBaseUrl}/images/default.png`}
          />
        </div>
          ...
          <div className="profile-image-selection">
            {profileImageOptions.map((imgPath) => (
              <img
                ...
                onClick={() => handleImageSelect(imgPath)} // Seçilen resmi anında günceller
                loading="lazy"
              />
            ))}
          </div>
