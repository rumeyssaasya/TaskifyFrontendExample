...
const Tasks = () => {
  ...
  const [tasksWithoutProject, setTasksWithoutProject] = useState<Task[]>([]);
  ...
  // Proje atanmamış görevleri filtrele
  useEffect(() => {
    setTasksWithoutProject(allTasks.filter(task => task.projectId === null));
  }, [allTasks]);
  ...
  const handleDrop = async (e: React.DragEvent, targetStatus: "TO_DO" | "IN_PROGRESS" | "DONE") => {
    ...
    try {
      ...
      await fetchTasks(); // Tüm görevleri yeniden çek
    } catch (err) {
  ...
  const getColumnTasks = (status: string) => {
    return tasksWithoutProject.filter(task => task.status === status);
  };
                ...
                {getColumnTasks(column.status).map((task) => (
                  ...
                  >
                    ...
                    <span className={`task-status ${task.status.toLowerCase().replace('_', '-')}`}>
                      {column.title} {/* Doğrudan sütun başlığını kullan */}
                    </span>
                  </div>
                ))}
              </div>
